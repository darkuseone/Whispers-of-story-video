...
AXES = {
    ...
    "overlay_kind":     dict(kind="pick", weight=0.9,
                             # mist убран: медленный «дым» поверх кадра
                             # делал ролики слишком похожими и жалобы
                             # зрителей. none чаще — чистая картинка.
                             options=["none", "none", "none", "motes",
                                      "motes", "stars", "sparks"]),
    ...
    # ── звук ──────────────────────────────────────────────────────────
    # duck_depth — насколько глубоко музыка уходит под голосом (ratio
    # компрессора). duck_style — манера ВОЗВРАТА: раньше ось выбирала,
    # где расставить ямы по плану, теперь задаёт release сайдчейна (см.
    # render.DUCK_RELEASE_MS). Значения оставлены прежними: журнал канала
    # хранит их у старых роликов, и дистанция векторов должна считаться.
