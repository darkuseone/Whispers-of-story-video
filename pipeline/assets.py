...
def _stem(word: str) -> str:
    # ОДНО правило на отбор выдачи и подбор кадра — vet.stem (см. CLAUDE.md,
    # «Кадр ставится под фразу…»). Своя копия здесь резала бы слова иначе,
    # чем build.words_of, и «совпало на поиске» перестало бы значить
    # «совпадёт в монтаже».
    return vet.stem(word)
...
def _match_words(query: str, tags: str):
    ...
    # Слова сводятся к одной форме (_stem = vet.stem): запрос «shell
    # mounds» и теги «shell, mound» иначе совпадали одним словом из двух.
