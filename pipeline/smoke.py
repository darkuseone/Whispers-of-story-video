...
TOP_LEVEL_KEYS = {
    ...
    "reject",
    # материал, отсмотренный чатом (scout.py → gather_pinned)
    "pinned_clips", "pinned_archive",
    # шортсы: сколько и какие дополнительные окна (shorts.pick_windows)
    "n_shorts", "shorts_extra",
    # монтаж
...
def check_top_level(job):
    ...
    """
    # «*_note» — пояснение автора к соседнему полю (vet_context_note):
    # та же заметка для человека, что и поле с подчёркиванием в начале.
    unknown = sorted(k for k in job
                     if not k.startswith("_") and not k.endswith("_note")
                     and k not in TOP_LEVEL_KEYS)
    if unknown:
        raise SystemExit(
