...
2. ГЕНЕРАЦИЯ КОРОТКОГО ВИДЕО. Модели minimax-hailuo-2.3, kling-2.5,
   wan-2.2 — три из четырёх заказанных, seedance 1.5pro не отозвалась ни
   на одно из написаний пути, см. MISSING_MODELS.
   Только 2-3 секунды и только тогда, когда футажа по
...
# Заказана была ещё seedance 1.5pro, и её здесь НЕТ намеренно. Двадцать
# три написания пути дали 404: у этого ключа модель либо называется
# как-то совсем иначе, либо не открыта вовсе. Держать её в списке нельзя —
# жребий в generate_video выбирал бы мёртвую модель в четверти случаев и
# тратил на неё попытку молча.
#
# Если написание найдётся, вернуть модель одной переменной окружения,
# ничего не пересобирая:
#     MAGNIFIC_VIDEO_SLUGS={"seedance-1.5pro": "<путь>"}
# Проверить, что путь живой, — этапом `magnific` в Actions.
MISSING_MODELS = {"seedance-1.5pro": "путь не найден, см. SLUG_CANDIDATES"}
...
# ГЕНЕРАЦИЯ ВИДЕО ЗАПРЕЩЕНА ОТДЕЛЬНЫМ ФЛАГОМ, и это не то же самое, что
# MAGNIFIC_ENABLED. Переменная среды управляет источником целиком, её
# однажды вернули в "1" ради картинок — и вместе с картинками молча
# вернулась генерация видеовставок: на miami-tequesta-01 в пул футажа
# уехали 14 сгенерированных клипов, при заказанном «весь футаж настоящий,
# машина рисует только изображения». Одной переменной среды для двух
# разных решений мало, поэтому решение про видео живёт своей константой и
# переменной среды не управляется. Снимать его — только по прямой просьбе
# автора, и тогда менять здесь, а не в окружении.
VIDEO_GEN_ALLOWED = False
...
def generate_video(prompt: str, dst: Path,
    ...
    """
    if not VIDEO_GEN_ALLOWED:
        log("  ! генерация видео запрещена (VIDEO_GEN_ALLOWED): "
            "весь футаж ролика должен быть настоящей съёмкой")
        return False
    if dst.exists():
        return True
...
SLUG_CANDIDATES = {
    ...
    "kling-2.5": ["kling-v2-5-pro"],
    "wan-2.2": ["wan-v2-2-720p"],
    # Ненайденная модель (см. MISSING_MODELS). Кандидаты сохранены, чтобы
    # не перебирать заново то, что уже дало 404, если однажды соберёмся
    # искать снова.
    "seedance-1.5pro": ["seedance-pro-1-5-1080p", "seedance-v1-5-pro",
                        ...
                        "seedance", "seedance-pro-1-5",
                        "seedance-lite-1080p", "seedance-2-0-pro",
                        "seedance-1-5", "bytedance-seedance-pro-1-5"],
}
