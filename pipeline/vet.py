...
# Внутри «не рассуждающих» порядок идёт ПО ЦЕНЕ, а не по новизне, и это
# проверено счётом. На miami-tequesta-01 зрение съело 3.31 млн входных
# токенов: по grok-4.20 ($1.25/$2.50 за млн) это около четырёх долларов,
# по «самой последней» grok-4.6 ($2/$6) было бы около семи. Новее здесь
# не значит дешевле — значит дороже в полтора-три раза за тот же ответ
# «годится или нет». Grok 4.1 Fast ($0.20/$0.50) стоит первым: если он
# примет картинку на пробе, тот же ролик обойдётся примерно в доллар.
# Проверка боем в choose_model всё равно отсеет модель, которая зрения не
# умеет, поэтому дешёвым кандидатам ничего не стоит быть первыми.
# Закрепить конкретную модель по-прежнему можно полем vet_model.
MODEL_PREFERENCE = ("4.1-fast", "4-1-fast", "fast",
                    "non-reasoning", "nonreasoning", "non_reasoning",
...
PRICE_TABLE = (
    ...
    ("4-3",     (1.25, 2.50)),
    ("4.5",     (2.00, 6.00)),   # Grok 4.5 — выход втрое дороже 4.20
    ("4-5",     (2.00, 6.00)),
    ("4.6",     (2.00, 6.00)),   # Grok 4.6 — флагман сентября 2026, НЕ дешевле
    ("4-6",     (2.00, 6.00)),
    ("4.1",     (0.20, 0.50)),   # Grok 4.1 Fast — самый дешёвый на сегодня
    ("4-1",     (0.20, 0.50)),
)
...
def choose_model(im, topic, desc, key, want=None, period_context=None):
    ...
    # ЧТО ВООБЩЕ ДОСТУПНО НА АККАУНТЕ — в лог, с ценой каждой.
    # Раньше печаталась только победившая строка, и по логу нельзя было
    # понять ни какие модели были на выбор, ни почему взята эта: вопрос
    # «а не появилась ли модель новее/дешевле» упирался в консоль xAI.
    log("  зрение, доступные модели по предпочтению "
        "(цена $/млн вход-выход):")
    for name in cands[:8]:
