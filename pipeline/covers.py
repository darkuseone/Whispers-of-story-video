...
ТЕКСТ КЛАДЁТ PIL, а не модель (TEXT_BY_MODEL = False ниже): модель
рисует кадр с тёмной полосой сверху, жёлтый Anton поверх ставит
paint_hook. Так буквы канала побайтово одинаковы на всех выпусках, и
опечатка в хуке физически невозможна. До сентября 2026 текст рисовала
модель — переключатель и разбор в комментарии над TEXT_BY_MODEL.
...
#         уже работает в запасном пути без ключа. ТАК СЕЙЧАС.
# True  — буквы рисует модель вместе с кадром (TYPE_LOCK). Так канал
#         работал с самого начала и до сентября 2026.
#
# ПОЧЕМУ ПЕРЕКЛЮЧИЛИ. Диффузионные модели ошибаются в тексте не по
# невезению, а по статистике: на MARIO-Eval только 14.4% сырых картинок
# Flux-стека (на нём стоит grok-imagine) воспроизводят запрошенный текст
# в точности, и надёжность разваливается как раз на 4-5 словах — ровно
# там, где живёт cover_texts. А ПРОВЕРИТЬ ОПЕЧАТКУ ЗДЕСЬ НЕКОМУ:
# build_covers пишет файл, логирует килобайты и КЭШИРУЕТ его («уже есть,
# не трогаю»), на пиксели не смотрит никто. «THE 1983 CIA DOSSSIER»
# уехал бы на YouTube и остался там, пока человек не удалит jpg руками.
#
# Второй довод — бренд. У модели каждый раз свои буквы: другой вес,
# другой трекинг, другое место. Протокол обложек требует обратного —
# «менять кадр и хук; не менять буквы». paint_hook даёт побайтово
# одинаковый Anton и один и тот же #FFD400 на всех выпусках, а его
# бинарный поиск кегля физически гарантирует высоту капители, которой
# протокол требует на 120 px.
#
# Цена решения: наложенная плашка не «сидит» в сцене так, как текст,
# рождённый вместе с кадром. Снимается это не рендером текста, а тем,
# что PLATE_LOCK просит НЕ пустой прямоугольник, а тёмную малодетальную
# область, ПРИНАДЛЕЖАЩУЮ сцене (градиент неба, стена в тени, уход в
# чёрное) яркостью ниже 25%.
#
...
# И помнить про cover_prompts: свой промпт со вписанным хуком в этом
# режиме заставит модель нарисовать СВОИ буквы под нашими (self_check
# про это предупреждает).
TEXT_BY_MODEL = False
